/*
 ...
 Updated by
    ____                      __
   / __ )___  ____ __________/ /___  ____
  / __  / _ \/ __ `/ ___/ __  / __ \/ __ \
 / /_/ /  __/ /_/ / /  / /_/ / /_/ / / / /
/_____/\___/\__,_/_/   \__,_/\____/_/ /_/
...
function UPS(args) {
    ...
    function buildShipment(data, options) {
        ...
        if (data.ship_from) {
            ...
            if (data.ship_from.tax_id_type && tax_id_types[ data.ship_from.tax_id_type ]) {
                shipment.ShipFrom.TaxIDType = {
                    Code: data.ship_from.tax_id_type,
                    Description: tax_id_types[ data.ship_from.tax_id_type ],
                };
            }
        }
        ...
        for (let i = 0; i < data.packages.length; i++) {
            ...
            if (data.packages[ i ].reference_number && typeof data.packages[ i ].reference_number === 'string') {
                ...
                p[ 'Package' ][ '#list' ] = [ ];
                for (let j = 0; j < data.packages[ i ].reference_number.length; j++) {
                    const r = data.packages[ i ].reference_number[ j ];
                    let ref;
                    if (typeof r === 'string') {
                        ref = {
                            'ReferenceNumber': {
    ...
    function buildLabelSpecification(data, options) {
        let label;
        const label_type = data.label_type || 'GIF';
        switch (label_type) {
            case 'EPL':
                label = {
                    'LabelPrintMethod': {
    ...
    $scope.dimensionalWeight = function (weight, length, width, height) {
        const dimWeight = (length * width * height) / ($scope.options.imperial ? dimensional_weight_values.imperial : dimensional_weight_values.metric);
        if (dimWeight > weight) {
    ...
    $scope.buildAccessRequest = function (data, options) {
        const root = builder.create('AccessRequest', { headless: true });
    ...
    $scope.buildRatesRequest = function (data, options) {
        ...
        const root = builder.create('RatingServiceSelectionRequest', { headless: true });
        let request = {
            'Request': {
                'RequestAction': 'Rate',
    ...
    $scope.buildTrackingRequest = function (tracking_number, options) {
        if (!options) options = { };
        const root = builder.create('TrackRequest', { headless: true });
        let request = {
            'Request': {
                'RequestAction': 'Track',
    ...
    $scope.buildShipmentConfirmRequest = function (data, options) {
        ...
        if (!options) options = { };
        data.pickup_type = data.pickup_type || 'daily_pickup';
        const root = builder.create('ShipmentConfirmRequest', { headless: true });
        let request = {
            'Request': {
    ...
    $scope.buildShipmentAcceptRequest = function (shipment_digest, options) {
        if (!options) options = { };
        const root = builder.create('ShipmentAcceptRequest', { headless: true });
        let request = {
            'Request': {
    ...
    $scope.buildVoidShipmentRequest = function (data, options) {
        if (!options) options = { };
        const root = builder.create('VoidShipmentRequest', { headless: true });
        let request = {
            'Request': {
                'RequestAction': '1',
                'RequestOption': '1',
        ...
        if (typeof data === 'string') {
            ...
            if (data.tracking_numbers) {
                ...
                for (let i = 0; i < data.tracking_numbers.length; i++) {
    ...
    $scope.buildAddressValidationRequest = function (data, options) {
        if (!options) options = { };
        const root = builder.create('AddressValidationRequest', { headless: true });
        let request = {
            'Request': {
                'RequestAction': 'XAV',
    ...
    $scope.buildTimeInTransitRequest = function (data, options) {
        ...
        const root = builder.create('TimeInTransitRequest', { headless: true });
        let request = {
            'Request': {
    ...
    $scope.buildPickupRequest = function (data, options, resource, callback) {
        soap.createClient(path.join(__dirname, 'wsdl', 'pickup', 'Pickup.wsdl'), { endpoint: `https://${ hosts[ $scope.options.environment ] + resource.p }` }, (err, client) => {
            ...
            let params = {
                ...
                Notification: { // was spelled "Nofification"
            ...
            params.PickupPiece = [ ];
            for (let i = 0; i < data.pickup_piece.length; i++) {
            ...
            client.ProcessPickupCreation(params, (err, result) => {
                if ($scope.options.debug) {
                    parser.parseString(client.lastRequest, { explicitArray: false }, (err, debug) => {
                        console.log(util.inspect(debug, { depth: null }));
                    });
                }
                if (err) return callback(err.root.Envelope.Body.Fault.detail.Errors, null);
    ...
    $scope.buildPickupRateRequest = function (data, options, resource, callback) {
        soap.createClient(path.join(__dirname, 'wsdl', 'pickup', 'Pickup.wsdl'), { endpoint: `https://${ hosts[ $scope.options.environment ] + resource.p }` }, (err, client) => {
            ...
            client.ProcessPickupRate(params, (err, result) => {
                if ($scope.options.debug) {
                    parser.parseString(client.lastRequest, { explicitArray: false }, (err, debug) => {
                        console.log(util.inspect(debug, { depth: null }));
                    });
    ...
    $scope.buildCancelPickupRequest = function (data, options, resource, callback) {
        soap.createClient(path.join(__dirname, 'wsdl', 'freight_pickup', 'FreightPickup.wsdl'), { endpoint: `https://${ hosts[ $scope.options.environment ] + resource.p }` }, (err, client) => {
            if (err) return callback(err, null);
    ...
    $scope.buildFreightRateRequest = function (data, options, resource, callback) {
        soap.createClient(path.join(__dirname, 'wsdl', 'freight_rate', 'FreightRate.wsdl'), { endpoint: `https://${ hosts[ $scope.options.environment ] + resource.p }` }, (err, client) => {
            if (err) return callback(err, null);
            ...
            if (data.commodity instanceof Array) {
                params.Commodity = [ ];
                for (let i = 0; i < data.commodity.length; i++) {
                    const d = {
            ...
            client.ProcessFreightRate(params, (err, result) => {
                if ($scope.options.debug) {
                    parser.parseString(client.lastRequest, { explicitArray: false }, (err, debug) => {
                        console.log(util.inspect(debug, { depth: null }));
                    });
    ...
    $scope.buildFreightShipRequest = function (data, options, resource, callback) {
        //soap.createClient(path.join(__dirname,  'wsdl', 'freight_shipping', 'FreightShip.wsdl'), { ignoredNamespaces: {namespaces: [ 'fsp' ]} }, (err, client) => {
        //if (err) return callback(err, null);
        ...
        if (data.commodity instanceof Array) {
            ...
            if (!params.Shipment[ '#list' ]) params.Shipment[ '#list' ] = [ ];
            for (let i = 0; i < data.commodity.length; i++) {
                const d = {
        ...
        /*
        client.ProcessShipment(params, (err, result) => {
            if ($scope.options.debug) {
        ...
        */
        ...
        let header = '<?xml version="1.0" encoding="UTF-8"?><soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:error="http://www.ups.com/XMLSchema/XOLTWS/Error/v1.1" xmlns:upss="http://www.ups.com/XMLSchema/XOLTWS/UPSS/v1.0" xmlns:tns="http://www.ups.com/WSDL/XOLTWS/FreightShip/v1.0">';
        header += `<soap:Header><upss:UPSSecurity xmlns:upss="http://www.ups.com/XMLSchema/XOLTWS/UPSS/v1.0"><upss:UsernameToken><upss:Username>${ $scope.options.username }</upss:Username><upss:Password>${ $scope.options.password }</upss:Password></upss:UsernameToken><upss:ServiceAccessToken><upss:AccessLicenseNumber>${ $scope.options.access_key }</upss:AccessLicenseNumber></upss:ServiceAccessToken></upss:UPSSecurity></soap:Header>`;
        const body_request = builder.create('Request', { headless: true });
        const body_shipment = builder.create('Shipment', { headless: true });

        let body = header + `<soap:Body><FreightShipRequest xmlns="http://www.ups.com/XMLSchema/XOLTWS/FreightShip/v1.0">${ body_request.att('xmlns', 'http://www.ups.com/XMLSchema/XOLTWS/Common/v1.0').ele(params.Request).end() }`;
        ...
        doRequest({
            ...
            if (result.Envelope.Body.Fault) return callback(result.Envelope.Body.Fault.detail, null);
    ...
    function fixNSData(data) {
        ...
        for (i in data) {
            const newData = fixNSData(data[ i ]);
            if (i.indexOf(':')) {
                const fixedKey = i.split(':').slice(-1);
    ...
    $scope.handleFreightShipResponse = function (data, callback) {
        ...
        const fspData = data.Envelope.Body.FreightShipResponse;
    ...
    $scope.buildFreightPickupRequest = function (data, options, resource, callback) {
        soap.createClient(path.join(__dirname, 'wsdl', 'freight_pickup', 'FreightPickup.wsdl'), { endpoint: `https://${ hosts[ $scope.options.environment ] + resource.p }` }, (err, client) => {
            ...
            client.ProcessFreightPickup(params, (err, result) => {
                if ($scope.options.debug) {
                    parser.parseString(client.lastRequest, { explicitArray: false }, (err, debug) => {
                        console.log(util.inspect(debug, { depth: null }));
                    });
    ...
    $scope.buildCancelFreightPickupRequest = function (data, options, resource, callback) {
        soap.createClient(path.join(__dirname, 'wsdl', 'freight_pickup', 'FreightPickup.wsdl'), { endpoint: `https://${ hosts[ $scope.options.environment ] + resource.p }` }, function (err, client) {
            ...
            client.ProcessFreightCancelPickup(params, (err, result) => {
                if ($scope.options.debug) {
                    parser.parseString(client.lastRequest, { explicitArray: false }, (err, debug) => {
                        console.log(util.inspect(debug, { depth: null }));
                    });
    ...
    const resources = {
        ...
        confirm: {
            p: '/ups.app/xml/ShipConfirm',
            f: $scope.buildShipmentConfirmRequest,
            r: $scope.handleShipmentConfirmResponse,
        },
        accept: {
            p: '/ups.app/xml/ShipAccept',
            f: $scope.buildShipmentAcceptRequest,
            r: $scope.handleShipmentAcceptResponse,
        },
        void: { p: '/ups.app/xml/Void', f: $scope.buildVoidShipmentRequest, r: $scope.handleVoidShipmentResponse },
        address_validation: {
            p: '/ups.app/xml/XAV',
            f: $scope.buildAddressValidationRequest,
            r: $scope.handleAddressValidationResponse,
        },
        time_in_transit: {
            p: '/ups.app/xml/TimeInTransit',
            f: $scope.buildTimeInTransitRequest,
            r: $scope.handleTimeInTransitResponse,
        },
        pickup: { p: '/webservices/Pickup', f: $scope.buildPickupRequest, r: $scope.handlePickupResponse, wsdl: true },
        pickup_rate: {
            p: '/webservices/Pickup',
            f: $scope.buildPickupRateRequest,
            r: $scope.handlePickupRateResponse,
            wsdl: true,
        },
        cancel_pickup: {
            p: '/webservices/Pickup',
            f: $scope.buildCancelPickupRequest,
            r: $scope.handleCancelPickupResponse,
            wsdl: true,
        },
        freight_pickup: {
            p: '/webservices/FreightPickup',
            f: $scope.buildFreightPickupRequest,
            r: $scope.handleFreightPickupResponse,
            wsdl: true,
        },
        cancel_freight_pickup: {
            p: '/webservices/FreightPickup',
            f: $scope.buildCancelFreightPickupRequest,
            r: $scope.handleCancelFreightPickupResponse,
            wsdl: true,
        },
        freight_rate: {
            p: '/webservices/FreightRate',
            f: $scope.buildFreightRateRequest,
            r: $scope.handleFreightRateResponse,
            wsdl: true,
        },
        freight_ship: {
            p: '/webservices/FreightShip',
            f: $scope.buildFreightShipRequest,
            r: $scope.handleFreightShipResponse,
            wsdl: true,
        },
    };

    function doBuildParams(data, options, resource) {
        const authorize = $scope.buildAccessRequest(data, options);
        const body = `<?xml version="1.0" encoding="UTF-8" standalone="yes" ?>${ authorize }<?xml version="1.0" encoding="UTF-8" standalone="yes" ?>${ resource.f(data, options) }`;
        const params = {
            host: hosts[ $scope.options.environment ],
            path: resource.p,
            method: 'POST',
    ...
    function buildResourceFunction(i, resources) {
        if (i === 'rates') {
            return (data, options, callback) => {
                ...
                if (data.services && data.services.length > 0) {
                    ...
                    for (let j = 0; j < data.services.length; j++) {
                        ...
                        const opts = doBuildParams(newData, options, resources[ i ]);
                        doRequest(opts.params, opts.body, (err, res) => {
                            if (err) {
                                responses.push(err);
                            } else {
                                responses.push(res);
                ...
                const opts = doBuildParams(data, options, resources[ i ]);
            ...
            return (data, options, callback) => {
                ...
                resources[ i ].f(data, options, resources[ i ], (err, res) => {
                    if (err) return callback(err, null);
